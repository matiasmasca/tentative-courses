...
describe 'TentativeCourses' do
  describe 'Single Course' do
    describe 'has to have' do
      before do
        ...
        @tentative_courses = CourseManager.create_tentative_course(@teachers_list, @enrolled_students_list)
      end
      it 'has a teacher' do
      ...
      it 'has a list of enrolled' do
        course = @tentative_courses[:tentative_courses][@teacher].first
        expect(course.students_enrolled).must_equal @enrolled_students_list
      end
    end
    ...
    describe 'Match Schedules between 1 teacher and 1 student' do
      it 'has a Teacher with schedule that exact match with one Student so it must purpose 1 Course' do
        @teachers_list = [ Teacher.new("Drippy Dan", [["Friday", 19]]) ]
        @enrolled_students_list = [ Student.new("Adrian Droide", :single, "Beginner", [["Friday", 19]]) ]

        tentative_courses = CourseManager.create_tentative_course(@teachers_list, @enrolled_students_list)
        teacher = @teachers_list.first
      ...
      it 'has a Teacher with schedule of 2 days that match in 1 day with 1 Student so it must purpose 1 Course' do
        @teachers_list = [ Teacher.new("Drippy Dan", [["Monday", 19], ["Friday", 19]]) ]
        @enrolled_students_list = [ Student.new("Adrian Droide", :single, "Beginner", [["Friday", 19]]) ]
      ...
      it 'has a Teacher with schedule that match in 2 days with 1 Student so it must purpose 2 possible courses' do
        @teachers_list = [ Teacher.new("Drippy Dan", [["Monday", 17], ["Friday", 19]]) ]
        @enrolled_students_list = [ Student.new("Adrian Droide", :single, "Beginner", [["Monday", 17],["Friday", 19]]) ]
      ...
      it 'has a Teacher with schedule of 3 days that match in 2 days with 1 Student so it must purpose 2 possible courses' do
        @teachers_list = [ Teacher.new("Drippy Dan", [["Monday", 17],["Tuesday", 17], ["Friday", 19]]) ]
        @enrolled_students_list = [ Student.new("Adrian Droide", :single, "Beginner", [["Monday", 17],["Friday", 19]]) ]
      ...
      it 'has a Teacher doesn\'t match with the Student so it not purpose any course and add the student to the \"student_out\" list' do
        @teachers_list = [ Teacher.new("Drippy Dan", [["Monday", 17],["Tuesday", 17], ["Friday", 19]]) ]
        @enrolled_students_list = [ Student.new("Adrian Droide", :single, "Beginner", [["Monday", 9],["Friday", 9]]) ]
    ...
    it 'has only 1 student enrolled if it has single mode' do
      ...
      @enrolled_students_list << Student.new("Adrian Droide", :single, "Beginner", [["Friday", 19]])
      @enrolled_students_list << Student.new("Martin P Gado", :single, "Beginner", [["Friday", 19]])
  ...
  describe 'Group Course' do
    before do
      @teachers_list = []
      @teachers_list << Teacher.new("Frigid Bridget", [["Monday", 19],["Friday", 17]])

      @enrolled_students_list = []
      @enrolled_students_list << Student.new("José K. Lavera", :group, "Intermediate", [["Monday", 19]])
      @enrolled_students_list << Student.new("Ines Queleto", :group, "Intermediate", [["Monday", 19]])
      @enrolled_students_list << Student.new("Guillermo Nigote.", :group, "Intermediate", [["Monday", 19]])
      @enrolled_students_list << Student.new("Oscar Arrota", :group, "Intermediate", [["Monday", 19]])
      @enrolled_students_list << Student.new("Federico N. Gelado", :group, "Intermediate", [["Monday", 19]])
      @enrolled_students_list << Student.new("Ines Talactita", :group, "Intermediate", [["Monday", 19]])
    end
    ...
    it 'can have a maximum of 6 students enrolled' do
      @enrolled_students_list << Student.new("Agustin Flador", :group, "Intermediate", [["Monday", 19]])
      @enrolled_students_list << Student.new("Martin P Gado" , :group, "Intermediate", [["Monday", 19]])
    ...
    it 'has 10 enrolled students so create 2 tentative courses' do
      @enrolled_students_list << Student.new("Agustin Flador", :group, "Intermediate", [["Monday", 19]])
      @enrolled_students_list << Student.new("Martin P Gado" , :group, "Intermediate", [["Monday", 19]])
      @enrolled_students_list << Student.new("Juanca Nario"  , :group, "Intermediate", [["Monday", 19]])
      @enrolled_students_list << Student.new("Matias Tronauta", :group, "Intermediate", [["Monday", 19]])
    ...
    it 'has 10 enrolled students so create 2 tentative courses in different days' do
      @enrolled_students_list << Student.new("Agustin Flador", :group, "Intermediate", [["Friday", 17]])
      @enrolled_students_list << Student.new("Martin P Gado" , :group, "Intermediate", [["Friday", 17]])
      @enrolled_students_list << Student.new("Juanca Nario"  , :group, "Intermediate", [["Friday", 17]])
      @enrolled_students_list << Student.new("Matias Tronauta", :group, "Intermediate", [["Friday", 17]])
    ...
    it 'has a Teacher without match with any Student' do
      @another_enrolled_students_list = []
      @another_enrolled_students_list << Student.new("Agustin Flador", :group, "Intermediate", [["Friday", 9]])
      @another_enrolled_students_list << Student.new("Martin P Gado" , :group, "Intermediate", [["Friday", 10]])
      @another_enrolled_students_list << Student.new("Juanca Nario"  , :group, "Intermediate", [["Friday", 11]])
      @another_enrolled_students_list << Student.new("Matias Tronauta", :group, "Intermediate", [["Friday", 12]])

      tentative_courses = CourseManager.create_tentative_course(@teachers_list, @another_enrolled_students_list)
  ...
  describe 'Multiple Teachers and Students' do
    before do
      ...
      @teachers_list = [ @teacher_one, @teacher_two, @teacher_three, @teacher_four, @teacher_five ]
      ...
      @enrolled_student_ten = Student.new("Juanca Nario", :group, "Pre-Intermediate", [["Monday", 19],["Friday", 9] ])
      ...
      @enrolled_student_thirteen = Student.new("Stefi Ebre", :group, "Advanced", [["Monday", 9], ["Tuesday", 9], ["Wednesday", 9], ["Thursday", 9], ["Friday", 9]])

      @enrolled_students_list = [@enrolled_student_one, @enrolled_student_two, @enrolled_student_three, @enrolled_student_four, @enrolled_student_five, @enrolled_student_six, @enrolled_student_seven, @enrolled_student_eight, @enrolled_student_nine, @enrolled_student_ten, @enrolled_student_eleven, @enrolled_student_twelve, @enrolled_student_thirteen]
    end

    it 'has a tentative course for all students but one' do
      draw_match_result = CourseManager.create_tentative_course(@teachers_list, @enrolled_students_list)
